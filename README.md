...
[![Docs](https://img.shields.io/badge/Docs-Kraken.Net-1b7f50?style=for-the-badge)](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=Kraken.Net)
...
The [Kraken.Net documentation](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=Kraken.Net) is the main resource for installing, configuring, and using the library.

| Resource | Description |
|--|--|
| [Client guide](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=Kraken.Net) | Installation, REST and WebSocket clients, authentication, dependency injection, error handling, and advanced features |
| [Examples](https://cryptoexchange.jkorf.dev/docs/exchange-clients/examples?library=Kraken.Net) | Common REST and WebSocket operations |
| [API reference](https://cryptoexchange.jkorf.dev/docs/exchange-clients/reference?library=Kraken.Net) | Client interfaces, methods, and properties |
...
For more examples and explanations, continue with the [Kraken.Net documentation](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=Kraken.Net) or browse the [compilable repository examples](https://github.com/JKorf/Kraken.Net/tree/master/Examples).
...
V2 uses a strict interface per capability. The `SharedApi` property on each API surface therefore exposes only the operations or subscriptions that surface actually supports. `IKrakenSharedApiClient` groups the exchange's Shared API surfaces for dependency injection and runtime capability lookup.
...
using var restClient = new KrakenRestClient();
...
The request and response models belong to `CryptoExchange.Net.SharedApis`, so the same operation can accept another exchange's `IGetTickerRest` implementation. When using dependency injection, inject `IKrakenSharedApiClient` to access all of the exchange's Shared API surfaces or inject a capability such as `IGetTickerRest` directly. Use `GetCapability` on the aggregate when the operation, transport, or trading mode is selected at runtime.
...
* Version 8.5.0 - 24 Sep 2026
    ...
    * Shared APIs
      ...
      * Added `(I)KrakenSharedApiClient` containing all V2 SharedApi references
      * Added `SharedApi` to `KrakenOptions` to provide SharedApi related options
    ...
    * Added FuturesApi.Account.WithdrawToSpotWalletAsync endpoint
    * Added FuturesApi.Account.GetPositionEventsAsync endpoint
    * Added FuturesApi.ExchangeData.GetTradingSymbolsAsync endpoint for user available symbols
    * Updated FuturesApi.Trading.SetLeverageAsync maxLeverage parameter to nullable
    * Removed CreateUserFuturesDataTracker from tracker factory; implementation could not work correctly

* Version 8.4.0 - 21 Aug 2026
    * Updated to CryptoExchange.Net v12.5.0
    * Added MakerFeePercentage, TakerFeePercentage to SharedSpotSymbol mapping
    * Added Shared ISpotOrderManagementSocketClient implementation

* Version 8.3.0 - 29 Jul 2026
